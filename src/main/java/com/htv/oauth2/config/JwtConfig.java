...
//    String issuer;
//
//    @ConfigProperty(name = "quarkus.smallrye-jwt.path.private-key", defaultValue = "keys/private_key.pem")
...
//        log.info("Public Key Location: {}", publicKeyLocation);
...
//            is.close();
//
//            log.info("Key content length: {} bytes", content.length());
//            log.info("First 50 chars: {}", content.substring(0, Math.min(50, content.length())));
//
//            // Verify format
...
//                    throw new IllegalStateException("Private key must be in PKCS#8 format. Use: openssl pkcs8 -topk8 ...");
//                } else {
